...
data_jags = as.list(dat[, c("log2myct", "log2mmin", "log2mmax","cache", "chmin", "chmax", "log2prp")])
params = c("int", "b_myct", "b_mmin", "b_mmax", "b_chmin", "b_chmax", "b_cache", "sig2")

lmlog_string = " model {
  for (i in 1:length(log2prp)) {
    log2prp[i] ~ dnorm(mu[i], prec)
    mu[i] = int + b_myct * log2myct[i] + b_mmin*log2mmin[i] + b_mmax*log2mmax[i] + b_chmin*chmin[i] + b_chmax *chmax[i] + b_cache * cache[i]
  }
...
X = as.matrix(dat[c(13,14,15,6, 7,8)])
# head(X)
